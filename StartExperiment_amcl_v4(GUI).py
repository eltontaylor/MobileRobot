...
class Experiment(object):
    ...
    def updatetrial(self):
        """ Update trial """
        GetGUI()
        delay = np.random.uniform(minimum,maximum)
...
def get_xy_loc(msg):
        ...
        size_x_map = 3328 #in pixel 1728 no difference when changed 3328
        size_y_map = 1952 #in pixel 1156 no difference when changed 1952
        locs_x_map_origin = -31.4 #in meter   //map7.yaml parameters (-31.4)
        locs_y_map_origin = -23.4 #in meter  //map7.yaml parameters (-23.4)
...
def GetGUI():
    ...
    maze_angleTolerance = GUIsettings.get("maze_angleTolerance")*DegToRad #convert degree to radian
    maze_destinationDuration = GUIsettings.get("maze_destinationDuration") #seconds
...
if __name__ == '__main__':
    ...
    MonkeyName = 'Chimpian'
    NumberOfTrials = maze_NumberOfTrials
    RewardTimeOut = maze_trialDuration # In seconds
    Orientation=0
    Angle_tolerance = maze_angleTolerance
    ...
    Experiment(NumberOfTrials=NumberOfTrials,
               RewardTimeOut= RewardTimeOut,
               Orientation=0,
               Angle_tolerance=Angle_tolerance, #Tolerance for angle of deviation, in rads
